...
AC_DEFUN([AX_PYTHON_MODULE_VERSION], [
    ...
    AS_IF([test $? -eq 0], [], [
        eval AS_TR_CPP(HAVE_PYMOD_$1_VERSION)=no
        AC_MSG_RESULT([no])
        AC_MSG_WARN([You need at least version $2 of the $1 Python module.])
    ])
    eval AS_TR_CPP(HAVE_PYMOD_$1_VERSION)=yes
    AC_MSG_RESULT([yes])
])
