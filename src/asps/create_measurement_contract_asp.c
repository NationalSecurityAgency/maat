                         ...
                         char *tpmpass, char *akctx)
{
    ...
    ret = sign_xml(doc, subcontract_node, scratch, keyfile, keypass, nonce, tpmpass,
                   akctx, sign_tpm ? SIGNATURE_TPM : SIGNATURE_OPENSSL);
    if(ret != 0) {
...
int asp_measure(int argc, char *argv[])
{
    ...
    if((argc < 12) ||
            ...
            ((tpmpass    = argv[7]) == NULL) ||
            ((akctx      = argv[8]) == NULL) ||
            (((sign_tpm   = strtol(argv[9], NULL, 10)) < 0) || errno != 0) ||
            (((compressed = strtol(argv[10], NULL, 10)) < 0) || errno != 0) ||
            (((encrypted  = strtol(argv[11], NULL, 10)) < 0) || errno != 0)) {
