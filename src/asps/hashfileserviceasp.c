...
/*! \file
 * This ASP is an example of a service asp that hashes the file sent in.
...
int asp_measure(int argc, char *argv[])
{
    ...
    sha1hash_data = (sha1hash_measurement_data*)sha1hash_measurement_type.alloc_data();
    if (!sha1hash_data) {
    ...
    if(__libmaat_debug_level >= 2) {
        char hash_ascii_buf[SHA1HASH_LEN*2 + 1];
        int i;
        for(i=0; i<SHA1HASH_LEN; i++) {
            sprintf(&hash_ascii_buf[2*i], "%02hhx", sha1hash_data->sha1_hash[i]);
        }
        asp_loginfo("File %s sha1 hash is %s\n", path, hash_ascii_buf);
    }
    ...
    if(measurement_node_add_rawdata(graph, node_id, &sha1hash_data->meas_data) < 0) {
