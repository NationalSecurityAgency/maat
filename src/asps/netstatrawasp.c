...
/*! \file
 * This ASP will collect RAW data similar to a normal call to netstat.
 */
...
#define ASP_NAME "netstatraw"

int asp_init(int argc, char *argv[])
{
    asp_loginfo("Initialized netstatraw ASP\n");
...
{
    asp_loginfo("Exiting netstatraw ASP\n");
    return ASP_APB_SUCCESS;
}

netstat_raw_line *chunk_line_data(char *raw_line)
{
    int rc;
    netstat_raw_line *ret = malloc(sizeof(netstat_raw_line));
    if (ret == NULL) {
        goto error;
    }
    memset(ret, 0, sizeof(netstat_raw_line));
    char *tmp;
    char addr[16] = {0};
    long int port;
    struct in_addr ip = {0};
    ...
    // Cast is safe because ports fit within a short
    ...
    if(rc < 0) {
        ...
        goto error;
    // Cast is justified because of bounds checking
    } else if((INT_MAX > SIZE_MAX && (unsigned int)rc > SIZE_MAX) || (size_t)rc >= sizeof(ret->rem_addr)) {
...
int asp_measure(int argc, char *argv[])
{
    ...
    if((file_address = (file_addr *)file_addr_space.alloc_address()) == NULL) {
        asp_logerror("Failed to allocate new file address for /proc/net/raw\n");
        unmap_measurement_graph(graph);
        return -1;
    }
    if(stat("/proc/net/raw", &file_stats) != 0) {
        asp_logerror("Failed to stat() /proc/net/raw\n");
    ...
    file_address->fullpath_file_name = strdup("/proc/net/raw");
    ...
    fp = fopen("/proc/net/raw", "r");
    if(fp == NULL) {
        dlog(0, "Error when trying to read /proc/net/raw\n");
        free_measurement_variable(var);
        unmap_measurement_graph(graph);
        return -1;
    }

    //graph_add_node(graph_id, var, NULL, &new_node);
    //graph_add_edge(graph_id, node_id, "raw children", new_node, &new_edge);
    ...
    netstat_raw_measurement_data *data = NULL;
    data = (netstat_raw_measurement_data *)netstat_raw_measurement_type.alloc_data();
