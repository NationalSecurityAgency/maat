...
/*! \file
 * This ASP will collect TCP6 data similar to a normal call to netstat.
 */
...
#define ASP_NAME "netstattcp6"

int asp_init(int argc, char *argv[])
{
    asp_loginfo("Initialized netstattcp6 ASP\n");
...
{
    asp_loginfo("Exiting netstattcp6 ASP\n");
    return ASP_APB_SUCCESS;
}

netstat_tcp6_line *chunk_line_data(char *raw_line)
{
    netstat_tcp6_line *ret = malloc(sizeof(netstat_tcp6_line));
    if (ret == NULL)
        return NULL;
    memset(ret, 0, sizeof(netstat_tcp6_line));
    char *tmp;
    ...
    // Cast is justified because the value of port should fit in a short
...
int asp_measure(int argc, char *argv[])
{
    ...
    if((file_address = (file_addr *)file_addr_space.alloc_address()) == NULL) {
        asp_logerror("Failed to allocate new file address for /proc/net/tcp6\n");
        unmap_measurement_graph(graph);
        return -1;
    }
    if(stat("/proc/net/tcp6", &file_stats) != 0) {
        asp_logerror("Failed to stat() /proc/net/tcp6\n");
        free_address(&file_address->address);
        unmap_measurement_graph(graph);
        return -1;
    }

    if(file_stats.st_size < 0 || (unsigned long long)file_stats.st_size > SIZE_MAX) {
    ...
    file_address->fullpath_file_name = strdup("/proc/net/tcp6");
    ...
    if(fp == NULL) {
        dlog(0, "Error when trying to read /proc/net/tcp6\n");
    ...
    netstat_tcp6_measurement_data *data = NULL;
    data = (netstat_tcp6_measurement_data *)netstat_tcp6_measurement_type.alloc_data();
