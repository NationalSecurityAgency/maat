 ...
 *
 * Usage: "ASP_NAME" <graph path> <peerchan> <partner_cert> <certfile> <keyfile> <keypass> <tpmpass> <akctx> <sign_tpm> <workdir>
 *
 ...
 * @tpmpass is the password for interaction with the TPM
...
static int sign_contract(xmlDoc *doc, xmlNode *subcontract_node, char *certfile,
                         char *keyfile, char *keypass, char *nonce, int sign_tpm, char *tpmpass, char *akctx)
{
    char *scratch;
    int ret = 0;
...
/**
 * @brief Create a measurement contract. In the long term, a seperate ASP should be used for
 *        this functionality
 *
...
static int future_create_msmt_contract_asp(char *workdir, char *certfile, char *keyfile,
        char *keypass, int sign_tpm, char *tpmpass, char *akctx,
...
int asp_measure(int argc, char *argv[])
{
    ...
    if((argc < 10) ||
            ...
            ((workdir       = argv[9])       == NULL)) {
        asp_logerror("Usage: "ASP_NAME" <graph path> <peerchan> <certfile> <keyfile> <keypass> <tpmpass> <akctx> <sign_tpm> <workdir> [partner_cert]\n");
        ret_val = -EINVAL;
        if (graph) {
    ...
    if(partner_cert) {
        ...
        ret_val = future_create_msmt_contract_asp(workdir, certfile,
                  keyfile, keypass, sign_tpm, tpmpass, akctx,
                  encbuf, encsize,
